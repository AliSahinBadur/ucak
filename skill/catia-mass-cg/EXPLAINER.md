...
| `calibrate` | Birimleri ve atalet konvansiyonunu **ölçer** | `units_profile.json` |
...
Bir adım insandan bilgi gerektiriyorsa (kalibrasyon bloğunun ölçüleri, araç
ve revizyon adı) `next_command` **boş** gelir ve yerine `ask_user_tr` ile
`command_template` gelir:

```json
{
  "next_command": null,
  "ask_user_tr": "Araç adını, varyantı ve revizyon numarasını kullanıcıya sorun...",
  "command_template": "python -m cmc extract --vehicle <ARAC> --variant <VARYANT> --revision <REVIZYON>"
}
```

Bu ayrım test sırasında ortaya çıktı ve önemli. İlk sürümde `next_command`
örnek değerlerle dolu bir komut veriyordu (`--vehicle ARAC --revision R01`).
Kural "next_command'i çalıştır" olduğu için küçük bir model bunu olduğu gibi
çalıştırır ve ölçüm yanlış araç adıyla kaydedilir. Çalıştırılabilir bir
komutta yer tutucu değer bırakmak, modelden onu fark etmesini beklemektir.
...
Hazır bir koşum takımı `runner/` altında: `agent.py` Ollama ile döngüyü
sürer ve komut izin listesi, yer tutucu kontrolü, insan onay kapısı ile
bağlam kırpmayı uygular; `run_evals.py` test vakalarını koşturup puanlar.
Ayrıntı için `runner/README.md`.
