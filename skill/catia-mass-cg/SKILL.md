...
3. **Her komutun çıktısındaki `next_command` alanını KENDİN çalıştır.**
   Boşsa dur. Komutu kullanıcıya yazma, "şu komutu çalıştırın" deme;
   kullanıcı komut çalıştırmıyor, sen çalıştırıyorsun. Komutu `run_cmc`
   aracına ver.
   İstisna: çıktıda `ask_user_tr` varsa, orada yazılanı kullanıcıya sor,
   cevabı `command_template` içindeki `<...>` yerlerine koy ve o komutu
   çalıştır. Şablondaki değerleri kendin doldurma, kullanıcıdan al.
...
```
python -m cmc doctor
python -m cmc calibrate --length <L> --width <W> --height <H> --density <kg/m3>
