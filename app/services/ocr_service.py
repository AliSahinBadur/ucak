...
class SelectiveOCRService:
    ...
    def __init__(
        ...
        *,
        ...
        dpi: int = OCR_DPI,
        min_text_characters: int = OCR_MIN_TEXT_CHARACTERS,
        tessdata_dir: str | Path | None = OCR_TESSDATA_DIR or None,
    ) -> None:
        ...
        self.languages = self._available_language_spec(languages, self.tessdata_dir)
    ...
    def _resolve_tessdata_dir(cls, configured: str | Path | None) -> Path | None:
        ...
        if OCR_TESSERACT_CMD:
            candidates.append(Path(OCR_TESSERACT_CMD).expanduser().parent / "tessdata")
